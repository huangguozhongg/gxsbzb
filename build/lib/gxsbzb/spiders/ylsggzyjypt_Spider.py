...
# 通知公告，法规制度
class ylsggzyjypt_tf_Spider(Spider):

    # 爬虫名
    name = 'ylsggzyjypt_tf'
    # 网站标题
    __WebTitle = '广西玉林市公共资源交易平台'
    # 网站地址
    __WebUrl = 'http://ggzy.yulin.gov.cn/'
    # 网站节点
    __WebNodes = [
                  '通知公告',
                  '法规制度'
    ...
    # 网站节点地址
    __WebNodeUrls = [
                       'http://ggzy.yulin.gov.cn/tzgg/%s.html',
                       'http://ggzy.yulin.gov.cn/fgzd/%s.html'
                       ]
    ...
    def parse(self, response):
        ...
        for WebNode,WebNodeUrl in zip(range(len(self.__WebNodes)),self.__WebNodeUrls):
            yield Request(WebNodeUrl % 'moreinfoonly', callback=self.__page_parse, method='GET', meta={'cookiejar': response.meta['cookiejar'], 'WebNode': WebNode, 'page':1})

    # 页面解析
    def __page_parse(self, response):
        try:
            # 抽取所有页面
            papers = response.xpath('//ul[@id="content_body"]/li[@class="ewb-info-item clearfix"]')
            # 抽取页面每一
            for paper in papers:
                # 标题
...
# 交易信息
class ylsggzyjypt_j_Spider(Spider):
    ...
    # 网站节点地址
    __WebNodeUrls = [
                       'http://ggzy.yulin.gov.cn/jyxx/002001/002001001/%s.html',
                       'http://ggzy.yulin.gov.cn/jyxx/002002/002002001/%s.html'
                       ]
    ...
    def parse(self, response):
        # 请求队列
        for WebNode,WebNodeUrl in zip(range(len(self.__WebNodes)),self.__WebNodeUrls):
            yield Request(WebNodeUrl % 'subMoreInfo', callback=self.__page_parse, method='GET', meta={'cookiejar': response.meta['cookiejar'], 'WebNode': WebNode, 'page':1})

    # 页面解析
    def __page_parse(self, response):
        try:
            # 抽取所有页面
            papers = response.xpath('//ul[@class="ewb-info-items"]/li')
            # 抽取页面每一
            for paper in papers:
                # 标题
                Title = paper.xpath('.//a/text()').extract()[0]
