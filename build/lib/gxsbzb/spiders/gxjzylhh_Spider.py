...
# 通知公告,社会咨询，政策法规
class gxjzylhh_tsz_Spider(Spider):
    ...
    __WebTitle = '广西建筑业联合会'
    # 网站地址
    __WebUrl = 'http://www.gxjzy.com.cn/'
    # 网站节点
    __WebNodes = ['通知公告',
                  '协会资讯',
                  '行业动态',
    ...
    # 网站节点地址
    __WebNodeUrls = [  'http://www.gxjzy.com.cn/TongZhiGongGao/%s.html',
                       'http://www.gxjzy.com.cn/XieHuiZhiXun/%s.html',
                       'http://www.gxjzy.com.cn/HangYeDongTai/%s.html',
                       'http://www.gxjzy.com.cn/ZhengCeFaGui/%s.html'
                       ]
    ...
    def __page_parse(self,response):
        try:
            # 首先抽取所有的文章
            papers = response.xpath('//div[@class="articlelist1"]//ul//li')
            # 从每篇文章中抽取数据
            for paper in papers:
                # 标题
                Title = paper.xpath('./a[last()]/text()').extract()[0].strip()
                # url
                Url = response.urljoin(paper.xpath('./a[last()]/@href').extract()[0].strip())
                # 日期
            ...
            # 提交request
            yield Request(url=self.__WebNodeUrls[response.meta['WebNode']] % (response.meta['page'] + 1), callback=self.__page_parse, meta={'WebNode':response.meta['WebNode'], 'page':(response.meta['page'] + 1)})