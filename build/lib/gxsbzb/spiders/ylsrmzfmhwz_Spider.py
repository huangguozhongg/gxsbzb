...
# 通知公告
class ylsrmzfmhwz_t_Spider(Spider):

    # 爬虫名
    name = 'ylsrmzfmhwz_t'
    # 网站标题
    __WebTitle = '广西玉林市人民政府门户网站'
    # 网站地址
    __WebUrl = 'http://www.yulin.gov.cn/web/ylmh/index.ptl'
    # 网站节点
    __WebNodes = [
                  '通知公告'
                  ]
    ...
    # 网站节点地址
    __WebNodeUrls = [
                       'http://www.yulin.gov.cn/menhuwangzhan/zfgzdt/tzgg/tzgg',
                       ]
    ...
    def __page_parse(self, response):
        try:
            # 抽取所有页面
            papers = response.xpath('//ul[@class="list"]/li')
            # 抽取页面每一
            for paper in papers:
                # 标题
            ...
            form_data = {
                'pageNo': '%s' % (response.meta['page'] + 1),
                'channelCode': response.xpath('//input[@id="currentChannelCode"]/@value').extract()[0],
                '$$time$$': datetime.today().strftime('%Y%m%d%H%M%S')
            }
            # 跟进请求
            yield FormRequest('http://www.yulin.gov.cn/web/ylmh/channel/channel.ptl', callback=self.__page_parse,formdata=form_data, method='GET', meta={'cookiejar': response.meta['cookiejar'], 'WebNode':response.meta['WebNode'], 'page':(response.meta['page'] +1)})
