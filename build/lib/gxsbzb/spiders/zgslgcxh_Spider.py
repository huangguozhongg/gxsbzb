...
# 通知公告，协会动态
class zgslgcxh_tx_Spider(Spider):
    ...
    # 网站节点地址
    __WebNodeUrls = [ 'http://www.cweun.org/list.php?id=11&kw=&page=%d',
                    'http://www.cweun.org/list.php?id=14&kw=&page=%d']


    # 第一个Request请求
    def start_requests(self):
        return  [Request('http://www.cweun.org/', callback=self.parse, method='GET', meta={'cookiejar':1})]
    ...
    def __page_parse(self, response):
        try:
            # 抽取所有页面
            papers = response.xpath('//table[@id="table15"]//tr')
            # 抽取页面每一
            for paper in papers:
                # 标题
                ...
                # 日期
                Time = paper.xpath('.//p/text()').extract()[0]
