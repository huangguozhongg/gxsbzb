...
# 通知公告
class gxcjrlhh_tzgg_Spider(Spider):

    # 爬虫名
    name = 'gxcjrlhh_tzgg'
    # 网站标题
    __WebTitle = '广西壮族自治区残疾人联合会'
    # 网站地址
    __WebUrl = 'http://www.gxdpf.org.cn/'
    # 网站节点
    __WebNodes = ['资讯频道通知公告']
    # 网站定位
    __Websites =__WebNodes
    # 网站节点地址
    __WebNodeUrls = [ 'http://www.gxdpf.org.cn/channels/646.html']
    ...
    def parse(self, response):
        try:
            # 抽取所有页面
            papers = response.xpath('//ul[@class="conList_ul"]/li')
            # 抽取页面每一
            for paper in papers:
                # 标题
            ...
            yield Request(response.urljoin(response.xpath('.//a[contains(@title,"下一页")]/@href').extract()[0]), callback=self.parse, meta={'cookiejar': response.meta['cookiejar']})
